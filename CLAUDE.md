...
45. **`updatePropertiesPanel()` must guard `selectedHotspot.line`**: A hotspot loaded from config.json without a `line` property crashes the panel; guard with `const segs = hotspot.line ? hotspot.line.segments : []`
46. **`positionTooltipForSegmentedLinePreview()` must guard empty segments**: `segments[0]` is `undefined` if array is empty — add `if (segments.length === 0) return;` before accessing elements
