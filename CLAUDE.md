...
55. **`loadConfiguration()` must deep-clone parsed JSON**: `this.hotspots = config.overlays || []` stores a direct reference — use `JSON.parse(JSON.stringify(config.overlays || []))` to prevent mutation of the originally-parsed object
